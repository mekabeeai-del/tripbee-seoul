"""
Google Query Rewriter - FIND_PLACE 의도 전용 쿼리 리라이트
...
class GoogleQueryRewriter:
    """FIND_PLACE 의도 전용 쿼리 리라이터 (Google Places API)"""
    ...
    def load_system_prompt(self):
        ...
        try:
            ...
            print(f"[GOOGLE_REWRITER] System prompt loaded from {prompt_path}")
        except Exception as e:
            print(f"[GOOGLE_REWRITER] Failed to load prompt file: {e}")
    ...
    def rewrite(
        ...
        """
        try:
            print(f"[GOOGLE_REWRITER] 쿼리 리라이트 시작: '{original_query}'")
...
브랜드명이 없을 때만 위치+카테고리 조합으로 만드세요."""
            ...
            if not function_call:
                print("[GOOGLE_REWRITER] Function call 없음, fallback 사용")
            ...
            print(f"[GOOGLE_REWRITER] 리라이트 성공:")
    ...
    def _fallback_rewrite(self, input_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            리라이트된 쿼리
        """
        print("[GOOGLE_REWRITER] Fallback 규칙 기반 리라이트 사용")
